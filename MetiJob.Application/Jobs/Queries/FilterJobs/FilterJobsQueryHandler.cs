...
namespace MetiJob.Application.Jobs.Queries.FilterJobs
{
    public class FilterJobsQueryHandler : IRequestHandler<FilterJobsQuery, OperationResult<FilterJobsResponse>>
    {
        ...
        public async Task<OperationResult<FilterJobsResponse>> Handle(FilterJobsQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                var query = _jobRepository.GetQuery().OrderByDescending(p => p.CreatedAt).AsQueryable();
                if (request.Dto.IsHot) query = query.Where(p => p.IsHot).AsQueryable();
                if (!string.IsNullOrEmpty(request.Dto.Title)) query = query.Where(p => p.Title.Contains(request.Dto.Title)).AsQueryable();
                if (!string.IsNullOrEmpty(request.Dto.CompanyName)) query = query.Where(p => p.Company.Name.Contains(request.Dto.CompanyName)).AsQueryable();
                ...
                if (request.Dto.MinPrice > -1) query = query.Where(p => p.MinPrice >= request.Dto.MinPrice).AsQueryable();
